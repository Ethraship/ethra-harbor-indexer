...
The HTTP server also serves a small local dashboard for viewing those read-only
metrics without a separate frontend app.
...
aggregation across multiple wallets, reward minting, WebSocket subscriptions, or
user-profile logic.

## Non-Goals

- No multi-wallet identity mapping or external account system
- No write API, admin panel, or authenticated operator console
