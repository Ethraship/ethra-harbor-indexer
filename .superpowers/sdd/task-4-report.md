...
- Changed `applyDeposit` and `applyWithdraw` to be lifetime-only metadata updates.
- Removed share balance, total supply, and reward-debt mutation from deposit and withdraw handling.
- Updated the ledger tests so share setup and burn behavior use `applyTransfer` mint/burn paths instead of `applyDeposit` / `applyWithdraw`.
- Added direct assertions that deposit and withdraw do not change `balanceRaw` or `totalSupplyRaw`.

### Commands run and results

- `node --import tsx --test test/ledger.test.ts` — pass
- `npm test` — pass, 58 tests passing
...
- No functional concerns from the fix itself.
- The repository still has unrelated untracked files that were not touched.
